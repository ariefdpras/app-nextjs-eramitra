...
export default class PaginationHandler extends Component {
    ...
    renderPageNumbers = (pageNumbers, totalPages) => {
        ...
        return (
            <Pagination>
                <Pagination.Prev disabled={active < 3} onClick={active > 3 && this.backHandler} />

                {
                    pageNumbers.map(number => {
                        if (
                            parseInt(active) > 3 && number >= parseInt(active) - 3 &&
                ...
                <Pagination.Next onClick={active <= totalPages - 3 && this.nextHandler} />
            </Pagination>
        );
    };
