...
async function getServerSideProps(req) {
  ...
  const getProducts = await fetch(`${"http://eramitra.com"}/api/getProduct?brand=${req.query.slug}`);
  const products = await getProducts.json();
  const getBrand = await fetch(`${"http://eramitra.com"}/api/getBrands/${req.query.slug}`);
