...
const Home = props => {
  ...
  const handleChange = (e, value) => {
    router.push(`${"https://staging.eramitra.com"}/products/brand/${props.query.slug}?page=${value}`);
    router.replace(`${"https://staging.eramitra.com"}/products/brand/${props.query.slug}?page=${value}`);
  };
...
async function getServerSideProps(req) {
  ...
  const getProducts = await fetch(`${"https://staging.eramitra.com"}/api/getProduct?brand=${req.query.slug}&active=true&page=${req.query.page - 1 || 0}`);
  const products = await getProducts.json();
  const getBrand = await fetch(`${"https://staging.eramitra.com"}/api/getBrands/${req.query.slug}`);
...
var __webpack_exports__ = __webpack_require__.X(0, [664,446,862,641,652], () => (__webpack_exec__(6703)));
