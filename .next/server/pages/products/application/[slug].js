...
async function getServerSideProps(req) {
  ...
  const getApplications = await fetch(`${"https://eramitra.com"}/api/getApplication`);
  const applications = await getApplications.json();
  const getProducts = await fetch(`${"https://eramitra.com"}/api/getProduct?application=${req.query.slug}`);
  const products = await getProducts.json();
  const getApplication = await fetch(`${"https://eramitra.com"}/api/getApplication/${req.query.slug}`);
